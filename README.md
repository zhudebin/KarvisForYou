...
> **⚠️ 这一步不能跳过！不配回调地址和可信 IP，Karvis 就无法正常工作。**

Karvis 启动后会**自动检测网络环境**，在终端打印回调地址。你只需要按下面 4 步操作：

#### 3.1 复制终端打印的回调地址

启动 `app.py` 后，终端会自动打印类似这样的横幅：
...
> 💡 **不需要手动安装任何内网穿透工具。** app.py 会自动处理：
> - 有公网 IP → 直接用 `http://你的IP:9000/wework`
> - 本地电脑 → **自动下载 cloudflared → 自动启动隧道 → 生成公网地址**（首次约 30MB，存到 `bin/`，之后秒启）
...
4. 填写：
   - **URL**：粘贴终端打印的回调地址（如 `https://xxx-xxx.trycloudflare.com/wework`）
   - **Token**：填 `.env` 里的 `WEWORK_TOKEN`
   - **EncodingAESKey**：填 `.env` 里的 `WEWORK_ENCODING_AES_KEY`
5. 点击 **保存** ✅

#### 3.3 配置企业可信 IP（重要！）

> **⚠️ 不配这个，Karvis 能收到消息但发不出回复！这是新手最常踩的坑。**

1. 在应用详情页，找到 **企业可信 IP**
2. 点击 **配置**
3. 填入你的 IP 地址：
   - **服务器部署**：填服务器的公网 IP（终端横幅里会提示）
   - **本地 + cloudflared 隧道**：打开 https://ifconfig.me ，把显示的 IP 填进去
4. 保存 ✅
...
> 💡 免费隧道地址每次重启会变。想固定地址，可登录 Cloudflare 创建命名隧道（免费），详见 [Cloudflare 文档](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/)。
